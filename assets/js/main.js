...
(function () {
  ...
  /* --------------------------------------------------------------------------
     1. Star Canvas — 300-particle interactive background
     -------------------------------------------------------------------------- */
  (function () {
    ...
    const cx_ = new Float32Array(N), cy_ = new Float32Array(N);
    const vx  = new Float32Array(N), vy  = new Float32Array(N);
    const sr  = new Float32Array(N);
    const sal = new Float32Array(N), stg = new Float32Array(N), ssp = new Float32Array(N);

    function initStar(i) {
      ox[i] = cx_[i] = Math.random() * window.innerWidth;
      oy[i] = cy_[i] = Math.random() * window.innerHeight;
      vx[i] = vy[i] = 0;
      sr[i]  = Math.random() * 1.3 + 0.2;
      sal[i] = Math.random() * .5 + .05;
      stg[i] = Math.random() * .55 + .05;
      ssp[i] = .003 + Math.random() * .01;
    }
    for (let i = 0; i < N; i++) initStar(i);

    const RR = 120, RF = .55, RE = .06, RD = .82;

    (function frame() {
      requestAnimationFrame(frame);
      ctx.clearRect(0, 0, W, H);
      for (let i = 0; i < N; i++) {
        sal[i] += (stg[i] - sal[i]) * ssp[i];
        if (Math.abs(sal[i] - stg[i]) < .008) stg[i] = Math.random() * .6 + .04;

        const dx = cx_[i] - mx, dy = cy_[i] - my;
        const dist = Math.hypot(dx, dy);
        if (dist < RR && dist > .5) {
          const f = (RR - dist) / RR * RF;
          vx[i] += (dx / dist) * f * 2.5;
          vy[i] += (dy / dist) * f * 2.5;
        }
        vx[i] += (ox[i] - cx_[i]) * RE;
        vy[i] += (oy[i] - cy_[i]) * RE;
        vx[i] *= RD; vy[i] *= RD;
        cx_[i] += vx[i]; cy_[i] += vy[i];

        const x = cx_[i], y = cy_[i], r = sr[i], a = sal[i];
        const g = ctx.createRadialGradient(x, y, 0, x, y, r * 1.8);
        g.addColorStop(0,   `rgba(255,210,100,${Math.min(a * 1.4, 1)})`);
        g.addColorStop(.25, `rgba(243,176,46,${a})`);
        g.addColorStop(.6,  `rgba(243,176,46,${(a * .18).toFixed(3)})`);
        g.addColorStop(1,   'rgba(243,176,46,0)');
        ctx.beginPath();
